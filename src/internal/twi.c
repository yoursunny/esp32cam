...
void
twi_setClock(unsigned int freq)
{
#if F_CPU == FCPU80
  if (freq <= 100000)
...
static bool
twi_write_byte(unsigned char byte)
{
...
unsigned char
twi_writeTo(unsigned char address, unsigned char* buf, unsigned int len,
...
unsigned char
twi_readFrom(unsigned char address, unsigned char* buf, unsigned int len,
             unsigned char sendStop)
{
  unsigned int i;
  if (!twi_write_start())
