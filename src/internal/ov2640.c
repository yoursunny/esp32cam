...
static const uint8_t svga_regs[][2] = {
  ...
  { SIZEL, ((SVGA_HSIZE >> 6) & 0x40) | ((SVGA_HSIZE & 0x7) << 3) |
             (SVGA_VSIZE & 0x7) },
...
set_framesize(sensor_t* sensor, framesize_t framesize)
{
  ...
  /* Write output width */
  ret |=
    SCCB_Write(sensor->slv_addr, ZMOW, (w >> 2) & 0xFF); // OUTW[7:0] (real/4)
  ret |=
    SCCB_Write(sensor->slv_addr, ZMOH, (h >> 2) & 0xFF); // OUTH[7:0] (real/4)
  ret |=
    SCCB_Write(sensor->slv_addr, ZMHH,
               ((h >> 8) & 0x04) | ((w >> 10) & 0x03)); // OUTH[8]/OUTW[9:8]
...
static int
set_framerate(sensor_t* sensor, framerate_t framerate)
{
  return 0;
}
...
set_contrast(sensor_t* sensor, int level)
{
  ...
  for (int i = 0; i < sizeof(contrast_regs[0]) / sizeof(contrast_regs[0][0]);
       i++) {
    ret |= SCCB_Write(sensor->slv_addr, contrast_regs[0][i],
...
set_brightness(sensor_t* sensor, int level)
{
  ...
  for (int i = 0;
       i < sizeof(brightness_regs[0]) / sizeof(brightness_regs[0][0]); i++) {
    ret |= SCCB_Write(sensor->slv_addr, brightness_regs[0][i],
...
set_saturation(sensor_t* sensor, int level)
{
  ...
  /* Write contrast registers */
  for (int i = 0;
       i < sizeof(saturation_regs[0]) / sizeof(saturation_regs[0][0]); i++) {
    ret |= SCCB_Write(sensor->slv_addr, saturation_regs[0][i],
...
static int
set_quality(sensor_t* sensor, int qs)
{
  int ret = 0;
...
static int
set_colorbar(sensor_t* sensor, int enable)
{
  int ret = 0;
  uint8_t reg;
